...
const app = express();
const http = require('http'); // for operating system-related utility methods and properties
...
io.sockets.on('connection', (socket) => {
    ...
    function log(...params) {
        const array = ['Message from server:', ...params];
    ...
    socket.on('create or join', (room) => {
        log(`Received requests to create or join room ${room}`);

        // Finding clients in the current room
        const clientsInRoom = io.sockets.adapter.rooms[room];
        const numClients =
              clientsInRoom ? Object.keys(clientsInRoom.sockets).length : 0;
        ...
        // If two clients are already in the room, do not add the current client in the room
